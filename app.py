    ...
    def generate_pdf_worker(self, configs, max_pages, save_path, page_w_mm, page_h_mm):
        try:
            ...
            for page_idx in range(max_pages):
                ...
                # --- ส่วนที่แก้ไข: สร้างตัวแปรเก็บข้อมูลหลายคอลัมน์ ---
                current_page_text = ""
                current_file_name = "" 
                ...
                for cfg in configs:
                    ...
                    if cfg["type"] == "text":
                        ...
                        style = ParagraphStyle(name='CustomText', fontName=used_font, fontSize=cfg["font_size"], leading=cfg["font_size"] * 1.3, alignment=align_code, wordWrap='CJK')
                        ...
                        # --- ส่วนที่แก้ไข: ดึงชื่อไฟล์ออกมาเก็บไว้ ---
                        current_file_name = os.path.basename(item_data) 
                ...
                # --- ส่วนที่แก้ไข: เพิ่มชื่อไฟล์ลงใน list ข้อมูล Excel ---
                excel_data.append([page_idx + 1, current_file_name, current_page_text])
            ...
            if excel_data:
                ...
                # --- ส่วนที่แก้ไข: เพิ่มหัว Column 'ชื่อไฟล์' ---
                df_export = pd.DataFrame(excel_data, columns=["หน้า", "ชื่อไฟล์รูปภาพ", "ข้อความ"])
